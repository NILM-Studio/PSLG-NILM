...
当前流程改为预算内生成。先在训练时间段内构造一个按类别/模式分层的确定性顺序，各比例取该顺序的前缀，因此1%包含于2%、2%包含于5%。对每个比例，基元波形库和经验周期块结构只允许来自该比例已经选中的真实周期；生成数量与真实周期数量相同。A为预算内真实周期，B增加同等数量的传统增强周期，C增加同等数量的预算内生成周期。周期类别和物理模式在全局训练周期上拟合，因此仍使用预算外的训练结构信息。更上游的DETSEC和KMeans沿用已有产物，时间划分发生在它们之后，当前没有证据证明它们只在训练时间段上拟合。预算来源检查通过不能等同于端到端无泄漏，也不能声称所有学习步骤仅使用该比例的标签预算。
...
#### 2026-09-14 恢复工作与审计修正

- 最近已推送的实验实现为 `89858d7`，预算内生成后的服务器结果尚未回传；本地没有 `log/` 或 `output/` 实验产物。
- 修正旧审计对上游状态表示的未经验证断言，改为 `upstream_state_representation_scope: not_verified`。旧版 `legacy_global` 不再报告预算隔离通过。
- 新增独立来源审计脚本，核对嵌套预算、生成数量、逐块来源、生成文件与C组对应关系、连续数据集引用和相同OFF背景；文件存在性检查不等同于NPZ数值质量验证。
- 当前生成器在同类别/模式的预算子集中重采样，允许使用锚点本身；小预算下某组只有一个周期时，多样性仍需单独评价。这不是先前近邻数10消融的完全相同设置。
- 下一步顺序：服务器运行来源审计；确认A/B/C输入开机率；补齐上游表征拟合范围审计并确定预算定义；再运行独立输出目录的A/B/C对比，避免覆盖旧模型或把历史诊断当最终证据。

服务器执行以下命令，将终端输出发回即可，不需要复制完整JSON：

```bash
python scripts/audit_budget_dataset.py \
  --run-id ukdale_wm_primglr_detsec_3789
```

`budget_provenance_passed: true` 仅代表预算来源与文件引用检查通过；`upstream_representation_holdout: not_verified` 是当前真实限制，不应手工改为通过。若提示预算数据清单不存在，先执行6.3.6中的 `nilm_dataset,nilm_continuous` 重建命令。
