...
class DilatedResGroup(nn.Module):
    def __init__(
        self, c_in=30, c_inner=30, c_out=30, kernel_size=3, dilation_list=[1, 2, 3, 4]
    ):
        ...
        for i, dilation in enumerate(dilation_list):
            if i == 0:
                ...
                layers.append(
                    ResUnit(c_inner, c_inner, k=kernel_size, dilation=dilation)
                )
...
class AttentionBranch(nn.Module):
    def __init__(self, c_in=50, c_inner=50, c_out=50, kernel_size=3):
        ...
        self.conv1_1 = nn.Conv1d(
            in_channels=c_inner, out_channels=c_inner, kernel_size=1
        )
...
class DResNet(nn.Module):
    ...
    def __init__(self, window_size=128, c_in=1, kernel_size=3):
        ...
        self.first_group = DilatedResGroup(
            c_in=30,
            c_inner=30,
            c_out=30,
        ...
        self.second_group = DilatedResGroup(
            c_in=30,
            c_inner=40,
            c_out=40,
            kernel_size=kernel_size,
            dilation_list=[2, 2, 2, 2],
        )
        self.third_group = DilatedResGroup(
            c_in=40, c_inner=50, c_out=50, kernel_size=kernel_size, dilation_list=[3, 3]
        )
        self.fourth_group = DilatedResGroup(
            c_in=50,
            c_inner=50,
            c_out=50,
            kernel_size=kernel_size,
...
class DAResNet(nn.Module):
    ...
    def __init__(self, window_size=128, c_in=1, kernel_size=3):
        ...
        self.attention_branch = AttentionBranch(
            c_in=40, c_inner=40, c_out=50, kernel_size=kernel_size
        )
